...
def make_entry(frame, show=None):
    e = tk.Entry(frame, width=30, font=("Arial", 13), bg=CARD, fg=WHITE, insertbackground=WHITE, relief="flat", bd=8,
...
def make_button(frame, text, command, color=BTN1):
    tk.Button(frame, text=text, command=command, font=("Arial", 13, "bold"), bg=color, fg=WHITE, width=22, height=2,
...
tk.Label(frame_register, text="🧠 Addiction Recovery System", font=("Arial", 20, "bold"), bg=BG, fg=YELLOW).pack(pady=30)
tk.Label(frame_register, text="User Registration", font=("Arial", 15, "bold"), bg=BG, fg=WHITE).pack(pady=5)
...
def load_question():
    ...
    tk.Label(frame_questions, text="🧠 Addiction Recovery System", font=("Arial", 20, "bold"), bg=BG, fg=YELLOW).pack(
        pady=20)
    tk.Label(frame_questions, text=f"Question {q_number} of {len(questions)}", font=("Arial", 20, "bold"), bg=BG,
             fg=GREEN).pack(pady=5)
    tk.Label(frame_questions, text=q_text, wraplength=480, font=("Arial", 20), bg=BG, fg=WHITE).pack(pady=15)
    ...
    tk.Button(frame_questions, text="Next →", command=lambda: next_question(q_id), font=("Arial", 13, "bold"), bg=BTN1,
              fg=WHITE, width=22, height=2, relief="flat", cursor="hand2").pack(pady=25)
...
def show_result():
    ...
    tk.Label(frame_result, text="✅ Assessment Complete!", font=("Arial", 20, "bold"), bg=BG, fg=GREEN).pack(pady=25)
    tk.Label(frame_result, text=f"Total Score : {result[0]} / 80", font=("Arial", 15, "bold"), bg=BG, fg=YELLOW).pack(
        pady=10)
    tk.Label(frame_result, text=f"Addiction Level : {result[1]}", font=("Arial", 15, "bold"), bg=BG, fg=BTN1).pack(
        pady=10)
    tk.Label(frame_result, text=f"💡 Tips : {result[3]}", wraplength=480, font=("Arial", 13), bg=BG, fg=WHITE).pack(
        pady=10)
    tk.Label(frame_result, text="⚠️ Disclaimer : This is only a self-assessment\n"
                                "test. It is NOT a medical diagnosis.\n"
                                "Please consult a professional counselor\n"
                                "for proper guidance and support.", wraplength=480, font=("Arial", 11, "italic"), bg=BG,
             fg=GRAY).pack(pady=15)
    tk.Button(frame_result, text="🔄 New Assessment", command=show_register, font=("Arial", 13, "bold"), bg=BTN2,
              fg=WHITE, width=22, height=2, relief="flat", cursor="hand2").pack(pady=20)
...
tk.Label(frame_admin_login, text="🔐 Admin Login", font=("Arial", 20, "bold"), bg=BG, fg=YELLOW).pack(pady=40)
...
def show_analysis():
    ...
    # ── Scrollable canvas setup with responsive centering ──
    ...
    # Pack canvas and scrollbar with responsive behavior
    ...
    # ── Title ──
    tk.Label(content_frame, text="📊 Admin Analysis Dashboard", font=("Arial", 20, "bold"), bg=BG, fg=YELLOW).pack(
        pady=20)
    ...
    try:
        ...
        tk.Label(stats_container, text=f"👥 Total Users : {total_users}", font=("Arial", 13, "bold"), bg=BG,
                 fg=GREEN).pack(pady=6)
        ...
        tk.Label(stats_container, text=f"📈 Highest Score : {high} / 80", font=("Arial", 13, "bold"), bg=BG,
        ...
        tk.Label(stats_container, text=f"📉 Lowest Score : {low} / 80", font=("Arial", 13, "bold"), bg=BG,
                 fg=WHITE).pack(pady=6)
        ...
        try:
            ...
            avg_display = "0.00"
        tk.Label(stats_container, text=f"📊 Average Score : {avg_display} / 80", font=("Arial", 13, "bold"), bg=BG,
                 fg=WHITE).pack(pady=6)
        ...
        tk.Label(stats_container, text=f"📝 Total Tests Taken : {total_tests}", font=("Arial", 13, "bold"), bg=BG,
                 fg=WHITE).pack(pady=6)
        ...
        tk.Label(content_frame, text="🧩 Users per Addiction Type", font=("Arial", 14, "bold"), bg=BG, fg=YELLOW).pack(
            pady=(10, 6))
        ...
        if addiction_types:
            for addiction_type, count in addiction_types:
                ...
                tk.Label(row, text=f" {at_text}", font=("Arial", 12, "bold"), bg=CARD, fg=WHITE, anchor="w").pack(
                    side="left")
                tk.Label(row, text=f"{count} ", font=("Arial", 12, "bold"), bg=CARD, fg=GREEN, anchor="e").pack(
                    side="right")
        else:
            tk.Label(addiction_container, text="No data available", font=("Arial", 12), bg=BG, fg=GRAY).pack(pady=4)
        ...
        tk.Label(content_frame, text="📊 Addiction Level Distribution", font=("Arial", 14, "bold"), bg=BG,
                 fg=YELLOW).pack(pady=(10, 6))
        ...
        if addiction_levels:
            for level, count in addiction_levels:
                ...
                row.pack(fill="x", pady=4, padx=20, ipadx=10, ipady=6)
                tk.Label(row, text=f" {level_text}", font=("Arial", 12, "bold"), bg=CARD, fg=level_color,
                         anchor="w").pack(side="left")
                tk.Label(row, text=f"{count} ", font=("Arial", 12, "bold"), bg=CARD, fg=level_color, anchor="e").pack(
                    side="right")
        else:
            tk.Label(level_container, text="No data available", font=("Arial", 12), bg=BG, fg=GRAY).pack(pady=4)

    except Exception as e:
        error_label = tk.Label(content_frame, text="⚠️ Error loading analysis. Check database.",
                               font=("Arial", 12, "bold"), bg=BG, fg=BTN1)
    ...
    # ── Back Button ──
    tk.Button(content_frame, text="← Back", command=show_admin_login, font=("Arial", 13, "bold"), bg=BTN2, fg=WHITE,
